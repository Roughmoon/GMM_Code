...
class DataSet():
    ...
    def DataToArr(self):
        ...
        groupedarrays = [group.values[:, 1:].astype(np.float64) for _, group in grouped]
...
if __name__ == '__main__':
    ...
    dataset.loadData(sheetName="Sheet8")
    dataset.DataToArr()
    obj = Obj(M=dataset.groupedarrays, G=2)
    Phi, Gmm = obj.EM(10000)
    print('Phi:\n', Phi)
    print('Gmm:\n', Gmm)
    kmeans = Kmeans(Phi)
    kmeans.K_means_plus(clusters=3)
    PHI_new = kmeans.joint(CompanyCode=dataset.groupedarraysKeys)
    kmeans.save('D:\\PythonClass\\PythonProjects\\GMMCoding\\Zhu2.xlsx')
    print('轮廓系数为：', kmeans.silhouette())
